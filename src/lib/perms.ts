...
export function voice(interaction: ChatInputCommandInteraction) {
	function client() {
		...
		const missingPerms = (interaction.member as GuildMember).voice.channel?.permissionsFor(interaction.guild!.members.me!).missing(resolved)!;
	...
	function member(target?: GuildMember) {
		if (target && !target.voice.channel) return `${target.displayName} is not in a voice channel.`;
		if (!(interaction.member as GuildMember).voice.channel) return `$ou need to be in a voice channel.`;
	}

	function memberToMember(target: GuildMember) {
		if ((interaction.member as GuildMember).voice.channelId !== target.voice.channelId)
